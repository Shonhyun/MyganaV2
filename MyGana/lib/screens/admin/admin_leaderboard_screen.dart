...
class _AdminLeaderboardScreenState extends State<AdminLeaderboardScreen> {
  ...
  Widget _buildAdminHeader(BuildContext context, bool isDarkMode, Color primaryColor, LeaderboardData leaderboardData) {
    return Container(
  ...
  Widget _buildStatsCards(BuildContext context, bool isDarkMode, Color primaryColor, LeaderboardData leaderboardData) {
    final entries = leaderboardData.entries;
    final totalXp = entries.fold<int>(0, (sum, entry) => sum + entry.totalXp);
    final avgLevel = entries.isNotEmpty ? entries.fold<double>(0, (sum, entry) => sum + entry.level) / entries.length : 0.0;
    final activeUsers = entries.where((entry) => entry.lastActive.isAfter(DateTime.now().subtract(const Duration(days: 7)))).length;
  ...
  Widget _buildTopPerformers(BuildContext context, bool isDarkMode, Color primaryColor, LeaderboardData leaderboardData) {
  ...
  Widget _buildPodiumCard(
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: position == 1 ? Border.all(color: color, width: 2) : Border.all(
  ...
  Widget _buildLeaderboardList(BuildContext context, bool isDarkMode, Color primaryColor, LeaderboardData leaderboardData) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Row(
