...
class AdminLeaderboardService {
  ...
  Future<List<Map<String, dynamic>>> _getFirebaseLeaderboardDataExcludingAdmins({bool forceRefresh = false}) async {
    try {
      // Check cache first
      if (!forceRefresh &&
      ...
      for (final userId in userIds) {
        try {
          final userSnapshot = await _database.ref().child('users').child(userId).child('isAdmin').get();
      ...
      print('Found ${leaderboard.length} non-admin users in leaderboard');
  ...
  Stream<LeaderboardData> watchAdminLeaderboard() {
    return _database.ref().child('leaderboard').orderByChild('totalXp').onValue.asyncMap((event) async {
